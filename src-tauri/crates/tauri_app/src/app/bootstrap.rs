...
pub async fn run() {
    ...
    omega_drive_download::provider::init_http_client(keep_alive_s);
    omega_drive_upload::http_client::init_http_client(keep_alive_s);
    info!(" Data directory: {}", base_dir.display());
    ...
    let cache_db_conn = match omega_drive_db::cache::open_cache_db(&cache_db_path) {
        Ok(conn) => conn,
        Err(e) => {
            eprintln!("Error: Cannot open cache database: {e}");
            std::process::exit(1);
        }
    };
