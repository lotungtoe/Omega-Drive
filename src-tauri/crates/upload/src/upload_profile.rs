...
use omega_drive_gateway::core::services::ExtensionNormalizer;
use omega_drive_gateway::upload::upload_rules::UploadProfileRule;
use omega_drive_gateway::upload::upload_profile_selection::{UploadProfileCandidate, UploadProfileSelection};

pub fn prepare_upload_rules(
    normalizer: &dyn ExtensionNormalizer,
...
pub fn select_upload_profile(
    ...
    for rule in rules {
        if let Some(specificity) = verify_upload_rule(
            rule,
            candidate.file_type,
...
mod tests {
    ...
    impl ExtensionNormalizer for TestNormalizer {
        fn normalize_extensions(&self, list: &[String]) -> Vec<String> {
            list.iter().map(|e| e.trim_start_matches('.').to_lowercase()).collect()
        }
        fn validate_upload_plan(&self, _: &omega_drive_gateway::upload::upload_plan::UploadPlan) -> Result<(), String> { Ok(()) }
    }
    ...
    fn prepare_upload_rules_filters_invalid_profiles_and_normalizes_extensions() {
        ...
        let prepared = prepare_upload_rules(&TestNormalizer, rules, &valid_profile_ids);
